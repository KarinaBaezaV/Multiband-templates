...
base = pd.read_csv(path.join(local, "new_fits_ab.csv"))
...
for z in "griz":
    ...
    for u in "griz":
        if u != z:
            my_Y = f"C{u}"
            y = base[my_Y].values
            coefs, cov = curve_fit(linear_curve, x, y)
            ...
            if generate_plots:
                ...
                plt.savefig(path.join(local, "linear_fit_plots", f"{my_Y}({my_X}).png"))
                # plt.show()
                plt.clf()
                plt.close()
                # plt.show()
        ...
        for u in "griz":
            if u != z:
                ...
                if len(ya) > 10: # Custom??
                    ...
                    a_coefs, a_cov = curve_fit(linear_curve, xa2, ya)
                    a_unc = np.sqrt(np.diag(a_cov))
                    b_coefs, b_cov = curve_fit(linear_curve, xb2, yb)
                    ...
                    if generate_plots:
                        ...
                        plt.savefig(path.join(local, "linear_fit_plots", f"{my_Ya}({my_Xa}).png"))
                        # plt.show()
                        plt.clf()
                        plt.close()
                        ...
                        plt.savefig(path.join(local, "linear_fit_plots", f"{my_Yb}({my_Xb}).png"))
                        # plt.show()
                        plt.clf()
                        plt.close()

df_out = pd.DataFrame(data={"Y" : ys, "X" : xs, "Y_w0" : y_w0s, "X_w0" : x_w0s, "f_ord" : f_order, "sine" : sine, "cosine" : cosine, "a" : As, "a_e" : A_e, "b" : Bs, "b_e" : B_e})
df_out.to_csv(path.join(local, "linear_fits_ab.csv"), index = False)
